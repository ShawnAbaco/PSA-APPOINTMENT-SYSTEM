<div class="modal-content-wrapper">
    ...
    @if ($clientHistory && $clientHistory->count() > 1)
        <div class="row justify-content-center mt-4" style="margin-top: 24px;">
            <div class="col-lg-10">
                <div class="detail-card">
                    <div class="detail-card-header">
                                ...
                                <tbody>
                                    @foreach ($clientHistory as $history)
                                        <tr class="history-row">
                                            <td class="history-td">
                                                {{ $history->appointment?->appointment_number ?? 'N/A' }}</td>
                                            <td class="history-td">
                                                {{ $history->appointment?->appointment_date ? date('M d, Y', strtotime($history->appointment->appointment_date)) : 'N/A' }}
                                            </td>
                                            <td class="history-td">
                                                {{ $services[$history->service] ?? $history->service }}</td>
                                            <td class="history-td">
                                                <span
                                                    class="status-badge-modal {{ $history->appointment?->status === 'confirmed' ? 'confirmed' : 'other' }}">
