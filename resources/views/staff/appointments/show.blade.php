...
<div class="container-fluid">
    ...
    <div class="row">
        <div class="col-md-6">
            <div class="card mb-4">
                <div class="card-header"><h5>Appointment Info</h5></div>
                <div class="card-body">
                    ...
                    <p><strong>Status:</strong> <span class="status-badge status-{{ $appointment->status }}">{{ ucfirst($appointment->status) }}</span></p>
    ...
    <div class="card">
        ...
        <div class="card-body">
            <table class="table table-bordered">
                <thead><tr><th>#</th><th>Full Name</th><th>Sex</th><th>Birthdate</th><th>Service</th></tr></thead>
                <tbody>
...
$(document).ready(function() {
    $('.confirm-btn').click(function() {
        let id = $(this).data('id');
        $.ajax({
            url: `/staff/appointments/${id}/confirm`,
            method: 'PUT',
    ...
    $('.complete-btn').click(function() {
        let id = $(this).data('id');
        $.ajax({
            url: `/staff/appointments/${id}/complete`,
            method: 'PUT',
            data: { _token: '{{ csrf_token() }}' },
            success: function() { location.reload(); }
        });
    ...
    $('.cancel-btn').click(function() {
        if(confirm('Cancel this appointment?')) {
            let id = $(this).data('id');
            $.ajax({
                url: `/staff/appointments/${id}/cancel`,
                method: 'PUT',
                data: { _token: '{{ csrf_token() }}' },
                success: function() { location.reload(); }
            });
