...
@section('content')
                                    ...
                                    <tr class="appt-table-row" data-status="{{ $appointment->status }}"
                                        ...
                                        <td>
                                            <span class="appt-status {{ $appointment->status }}">
                                                <i
                                                    class="fas {{ $appointment->status == 'confirmed' ? 'fa-check-circle' : 'fa-clock' }}"></i>
                                        ...
                                        <td>
                                            <div class="appt-action-buttons">
                                                ...
                                                @if ($appointment->status == 'pending')
                                                    <button class="appt-btn-action appt-btn-confirm"
    ...
    <script>
        ...
        async function refreshTable() {
            ...
            try {
                const response = await fetch(`/staff/appointments?${params.toString()}`, {
                    headers: {
        ...
        async function confirmAppointment(id) {
            ...
            if (result.isConfirmed) {
                ...
                try {
                    const response = await fetch(`/staff/appointments/${id}/confirm`, {
                        method: 'PUT',
                        headers: {
        ...
        async function cancelAppointment(id) {
            ...
            if (result.isConfirmed) {
                ...
                try {
                    const response = await fetch(`/staff/appointments/${id}/cancel`, {
                        method: 'PUT',
                        headers: {
                            'Content-Type': 'application/json',
                            'X-CSRF-TOKEN': csrfToken
        ...
        async function deleteAppointment(id) {
            ...
            if (result.isConfirmed) {
                ...
                try {
                    const response = await fetch(`/staff/appointments/${id}`, {
                        method: 'DELETE',
                        headers: {
                            'Content-Type': 'application/json',
                            'X-CSRF-TOKEN': csrfToken
        ...
        async function bulkConfirm() {
            ...
            if (result.isConfirmed) {
                ...
                const promises = Array.from(selectedAppointments).map(id =>
                    fetch(`/staff/appointments/${id}/confirm`, {
                        method: 'PUT',
                        headers: {
                            'Content-Type': 'application/json',
                            'X-CSRF-TOKEN': csrfToken
        ...
        async function bulkCancel() {
            ...
            if (result.isConfirmed) {
                ...
                const promises = Array.from(selectedAppointments).map(id =>
                    fetch(`/staff/appointments/${id}/cancel`, {
                        method: 'PUT',
                        headers: {
                            'Content-Type': 'application/json',
                            'X-CSRF-TOKEN': csrfToken
        ...
        function exportToCSV() {
            const rows = document.querySelectorAll('.appt-table-row');
            const csvData = [
                ['Appointment #', 'Date', 'Time', 'Contact Name', 'Contact Phone', 'Status', 'Applicants']
            ];
        ...
        document.getElementById('quickAddForm')?.addEventListener('submit', async function(e) {
            ...
            try {
                const response = await fetch('/staff/appointments', {
                    method: 'POST',
                    body: formData
                });
        ...
        async function openEditModal(id) {
            ...
            try {
                const response = await fetch(`/staff/appointments/${id}/edit`, {
                    headers: {
                        'X-Requested-With': 'XMLHttpRequest',
                        'Accept': 'application/json'
                    }
                ...
                if (data.success && data.data) {
                    ...
                    <form id="editAppointmentForm" class="edit-form">
                        ...
                        <div class="edit-form-group">
                            <label class="edit-label">Applicants (${apt.clients ? apt.clients.length : 0})</label>
                            ${clientsHtml}
                        </div>
