...
private function formatTimeRange($startTime, $endTime)
{
    ...
    /**
     * Generate PDF from client PDF blade using posted appointment data
     * Expects JSON payload with 'appointment' and 'clients' keys
     */
    public function pdf(Request $request)
    {
        try {
            ...
            $appointment = $data['appointment'] ?? $data['appointment_data'] ?? [];
            $clients = $data['clients'] ?? $data['clients_list'] ?? ($appointment['clients_list'] ?? []);

            // Normalize clients to array of associative arrays expected by the blade
            // Ensure service_label and name keys exist
            $normalized = [];
            foreach ($clients as $c) {
                $name = $c['name'] ?? trim(($c['first_name'] ?? '') . ' ' . ($c['middle_name'] ?? '') . ' ' . ($c['last_name'] ?? ''));
                $normalized[] = array_merge($c, ['name' => $name, 'service_label' => $c['service_name'] ?? $c['service_label'] ?? $c['service'] ?? '']);
            }

            $viewData = [
                'appointment' => $appointment,
                'clients' => $normalized,
                'appointment_reference' => $appointment['reference_code'] ?? $appointment['reference'] ?? ($appointment['reference'] ?? ''),
                'schedule_display' => $appointment['date'] ?? ($appointment['schedule_display'] ?? ''),
                'contact_name' => $appointment['contact_name'] ?? ($appointment['contact_name'] ?? ''),
                'clients_list' => $normalized,
            ];

            $pdf = PDF::loadView('client.pdf', $viewData);

            $fileName = 'appointment-' . ($appointment['reference_code'] ?? $appointment['reference'] ?? date('YmdHis')) . '.pdf';
...
private function buildRequirementsHtml($service, $ageGroup, $requirements)
{
    ...
    if ($ageGroup === 'child') {
        $ageGroupLabel = '0-4 years old';
    } else {
        $ageGroupLabel = '5 years old and above';
    }
